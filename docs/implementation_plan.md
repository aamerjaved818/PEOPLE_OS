...
### Manual Verification
1. **User Creation**:
   - Create a new user in `OrgSetup` linked to an active employee.
   - Verify user appears in list and in DB (if I could access it, or via persistence check).
   - Refresh page to verify persistence.

2. **Access Control**:
   - Log in as the new user.
   - Login should success.
   - In `Employee` module, "Exit" the linked employee.
   - Verify `OrgUser` status updates to `Inactive` in `OrgSetup`.
   - Try to log in again as the user.
   - Login should FAIL with "Account is inactive" or similar.

3. **Profile Linking**:
   - Try to create a user without selecting an employee (UI should block).
   - Try to create a user linked to an *already linked* employee (UI should filter out).
