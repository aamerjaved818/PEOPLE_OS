...
def get_engine():
    ...
    # Attach SQLite pragma for foreign keys. For tests we disable FK enforcement
    # to allow lightweight fixtures to insert rows without creating all parent
    # rows (tests perform application-level validation). Tests set APP_ENV=test
    if "sqlite" in db_url:
        @event.listens_for(new_engine, "connect")
        def set_sqlite_pragma(dbapi_connection, connection_record):
            cursor = dbapi_connection.cursor()
            if ("APP_ENV" in __import__("os").environ and __import__("os").environ.get("APP_ENV") == "test"):
...
# Global instances for the package
engine = _EngineProxy()
...
def get_db():
    """FastAPI-style dependency that yields a DB session and ensures it is closed.

    Tests and other modules import `get_db` from `backend.database`.
    Provide a small generator wrapper around `SessionLocal()` for compatibility.
    """
    db = SessionLocal()
    try:
        yield db
