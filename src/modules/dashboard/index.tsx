    ...
    return (
      ...
      >
        ...
        <div className="relative z-10">
          <div
            className={`w-12 h-12 rounded-xl ${colors.bg}/10 ${colors.text} flex items-center justify-center mb-4 group-hover:scale-125 group-hover:${colors.bg}/20 transition-all duration-300 group-hover:shadow-lg group-hover:shadow-current/30`}
          >
            <Icon
              size={24}
              aria-hidden="true"
...
const DashboardWrapper: React.FC = () => (
  <ErrorBoundary>
    <div className="h-full w-full overflow-y-auto overscroll-contain custom-scrollbar px-6 md:px-10 pb-10 bg-noise">
    ...
    >
      ...
      <div>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-8">
          <div>
            <h1 className="text-5xl font-black text-vibrant tracking-tighter leading-none uppercase drop-shadow-sm">
              Dashboard
            </h1>
            <p className="text-muted-foreground mt-4 font-black uppercase tracking-[0.4em] text-[0.6rem] flex items-center gap-3">
              <span className="w-10 h-[2px] bg-primary/50"></span>
          ...
          <div className="flex flex-col items-end gap-3">
            ...
            <div className="flex items-center gap-4">
              <Button
                ...
                variant="outline"
                className="p-3 h-auto rounded-2xl hover:bg-muted transition-all group backdrop-blur-xl shadow-lg disabled:opacity-50 border-border"
                title="Refresh Data"
              ...
              <Button
                ...
                variant="outline"
                className="p-3 h-auto rounded-2xl hover:bg-muted border-border transition-all duration-300 group backdrop-blur-xl shadow-lg hover:shadow-xl hover:shadow-primary/30 hover:scale-110 hover:-translate-y-1"
              ...
              <div className="bg-card backdrop-blur-xl px-6 py-3 rounded-2xl shadow-2xl border border-border flex items-center gap-4">
                <div className="relative">
                  <div
                    className={`w-3 h-3 ${systemStatus === 'Optimal' ? 'bg-success' : systemStatus === 'Degraded' ? 'bg-warning' : 'bg-destructive'} rounded-full animate-ping absolute opacity-75`}
                  ></div>
                  <div
                    className={`w-3 h-3 ${systemStatus === 'Optimal' ? 'bg-success' : systemStatus === 'Degraded' ? 'bg-warning' : 'bg-destructive'} rounded-full relative shadow-[0_0_15px_rgba(16,185,129,0.3)]`}
                  ></div>
                </div>
                <span className="text-[0.6rem] font-black text-muted-foreground uppercase tracking-widest">
                  System Status:{' '}
                  <span className={systemStatus === 'Optimal' ? 'text-success' : 'text-warning'}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={idx}
            className="card-vibrant border-border/50 rounded-2xl p-6 hover:bg-accent/50 transition-all group hover:border-primary/50"
          >
      ...
      <div className="card-vibrant p-10 rounded-[3rem] text-card-foreground shadow-2xl relative overflow-hidden group border border-primary/20">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <h2 className="text-4xl font-black tracking-tighter leading-none mb-6 text-foreground">
              Workforce Analytics
            </h2>
            ...
            <div className="flex gap-4">
              <div className="px-6 py-3 bg-card border border-border rounded-2xl flex items-center gap-3">
                ...
                <span className="text-[0.625rem] font-black uppercase tracking-widest text-foreground">
                  Real-Time Analytics
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div
                key={i}
                className="p-6 bg-card/40 border border-border/50 rounded-2xl backdrop-blur-xl hover:bg-primary/10 hover:border-primary/50 transition-all text-center group/card shadow-sm"
              >
                <item.icon className="w-8 h-8 mx-auto mb-3 text-primary group-hover/card:scale-110 transition-transform" />
                <p className="text-[0.6875rem] font-black uppercase tracking-[0.2em] text-foreground">
                  {item.label}
                </p>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div
          className="xl:col-span-2 p-10 card-vibrant border-border/50 rounded-[3rem] hover:bg-accent/5 transition-all flex flex-col min-h-[28.125rem] group relative overflow-hidden"
          role="region"
        ...
        >
          ...
          <div className="relative z-10 flex items-center justify-between mb-8">
            ...
            <select
              className="bg-card border border-border backdrop-blur-xl rounded-xl text-[0.625rem] font-black uppercase tracking-wider px-4 py-2.5 outline-none text-foreground hover:bg-muted hover:border-border hover:shadow-lg hover:shadow-primary/30 transition-all duration-300 focus:ring-2 focus:ring-primary/50 cursor-pointer"
        ...
        >
          ...
          <div className="flex items-center gap-4 mb-8 relative z-10">
            <div className="w-12 h-12 bg-gradient-to-br from-primary to-primary/80 text-primary-foreground rounded-xl flex items-center justify-center shadow-lg shadow-primary/20">
              <Sparkles size={24} />
            </div>
            <div>
              <h3 className="text-xl font-bold text-foreground tracking-tight">Celebrations</h3>
              <p className="text-[0.625rem] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">
                Upcoming Milestones
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto pr-1 custom-scrollbar relative z-10 max-h-[21.875rem]">
            {milestones.map((m) => (
              <div
                key={m.id}
                className="p-4 bg-muted/30 rounded-xl border border-border/50 hover:bg-card hover:border-primary/30 hover:shadow-sm transition-all duration-300 group/item"
                role="article"
              ...
              >
                <div className="flex items-center gap-4 mb-4">
                  <div className="relative">
                    <img
                      src={m.avatar}
                      className="w-10 h-10 rounded-lg object-cover ring-2 ring-transparent group-hover/item:ring-primary/30 transition-all"
                      alt={`${m.name}'s profile picture`}
                    />
                    <div
                      className="absolute -bottom-1 -right-1 bg-card rounded-full p-0.5 border border-border"
                      aria-hidden="true"
                    >
                      {m.type === 'Birthday' ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className="text-[0.5625rem] font-black px-2 py-0.5 bg-primary/10 text-primary rounded-md uppercase tracking-wide"
                        aria-label={`Date: ${m.date}`}
                      >
                        {m.date}
                      </span>
                    </div>
                    <p className="text-[0.625rem] text-muted-foreground mt-0.5 truncate uppercase tracking-wide font-medium">
                      {m.type} • {m.detail}
                    </p>
                  </div>
                </div>
                <button
                  ...
                  className={`w-full py-2.5 rounded-lg text-[0.625rem] font-black uppercase tracking-[0.2em] transition-all duration-300 flex items-center justify-center gap-2 ${
                    wishesSent.includes(m.id)
                      ? 'bg-success text-success-foreground shadow-inner opacity-90'
                      : 'bg-card text-muted-foreground border border-border hover:bg-primary hover:text-primary-foreground shadow-sm hover:shadow-2xl hover:shadow-primary/40 hover:scale-105 hover:-translate-y-1'
                  }`}
                  aria-label={
                    wishesSent.includes(m.id)
                      ? `Wish sent to ${m.name}`
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        ...
        >
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="font-bold text-xl text-foreground tracking-tight">Departments</h3>
              <p className="text-[0.625rem] font-bold uppercase text-muted-foreground tracking-widest mt-0.5">
          ...
          <div className="flex-1 min-h-[15.625rem] relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="text-center">
                <span className="block text-2xl font-black text-foreground">{totalEmployees}</span>
                <span className="text-[0.5625rem] font-black uppercase text-muted-foreground tracking-widest">
                  Total
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            {deptStats.slice(0, 4).map((entry, index) => (
              <button
                ...
                className="flex items-center gap-2 hover:bg-muted/80 p-2 rounded-lg transition-all duration-300 group hover:pl-3 hover:shadow-md hover:shadow-primary/20"
              ...
              >
                ...
                <ChevronRight
                  className="w-3 h-3 text-muted-foreground opacity-0 group-hover:opacity-100 ml-auto transition-opacity"
                  aria-hidden="true"
                />
              </button>
            ))}
